...
\begin{tikzpicture}[node distance=2cm, auto]
    \node[rectangle, draw, fill=blue!20, text width=3cm, align=center, minimum height=1.5cm] (user) {Utilizator Final};
    \node[rectangle, draw, fill=blue!40, text width=4cm, align=center, minimum height=2cm, right=3cm of user] (system) {Unemployment Data Visualizer System};
    \node[rectangle, draw, fill=gray!20, text width=3cm, align=center, minimum height=1.5cm, right=3cm of system] (datagov) {data.gov.ro (External API)};

    \draw[->, thick] (user) -- node[above] {Vizualizează date} (system);
    \draw[->, thick] (system) -- node[above] {Preluare CSV} (datagov);
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[
    ...
    \node[comp, right=2cm of fetch] (cacheman) {CacheSystem};
    \node[comp, left=2cm of fetch] (models) {DTO Models};
...
\begin{enumerate}
    ...
    \item Proxy-ul PHP utilizează cURL pentru a comunica cu containerul \texttt{unemployment-api} prin DNS-ul intern al Docker.
...
\subsubsection{Endpoint: List Cache}
\begin{itemize}
    \item \textbf{Metodă}: \texttt{GET}
    \item \textbf{URL}: \texttt{/api/cache}
    \item \textbf{Răspuns}: Listă de obiecte \texttt{CsvCacheEntry} cu detalii despre mărime și data creării.
\end{itemize}

\subsubsection{Endpoint: Delete Cache}
\begin{itemize}
    \item \textbf{Metodă}: \texttt{DELETE}
    \item \textbf{URL}: \texttt{/api/cache} (Toate fișierele) sau \texttt{/api/cache/\{filename\}} (Un singur fișier).
    \item \textbf{Răspuns}: \texttt{\{"success": true, "msg": "..."\}}
\end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{int \$under25}, \texttt{\$from25to29}, \texttt{\$from30to39}, \texttt{\$from40to49}, \texttt{\$from50to59}, \texttt{\$over50}: Distribuție numerică pe vârste.
\end{itemize}
...
\begin{itemize}
    \item \texttt{string \$county}: Județ.
    \item \texttt{int \$noStudy}, \texttt{\$primaryStudy}, \texttt{\$middleStudy}, \texttt{\$highStudy}, \texttt{\$postHighStudy}, \texttt{\$professionalStudy}, \texttt{\$universityStudy}: Distribuție după nivelul de școlarizare.
\end{itemize}

\subsubsection{UnemploymentDataPerMedium}
Mapat pe \texttt{medii.csv}.
\begin{itemize}
    \item \texttt{int \$totalUnemployedUrban}, \texttt{\$totalUnemployedRural}: Distribuție Urban vs Rural.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Fonturi Custom}: Embedează fontul \textbf{Roboto} (Normal/Bold) sub formă de Base64 pentru suportul diacriticelor.
